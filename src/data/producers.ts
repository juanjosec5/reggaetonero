...
export const PRODUCERS = defineCatalog<ProducerDef>('producer', [
...
export function pickProducer(rng: Rng, opts: { genre?: Genre; maxCost?: number } = {}): ProducerDef | undefined {
  const affordable = PRODUCERS.all.filter((p) => opts.maxCost === undefined || p.cost <= opts.maxCost)
  if (affordable.length === 0) return PRODUCERS.all.reduce((a, b) => (b.cost < a.cost ? b : a))
  return weightedPick(
    affordable,
